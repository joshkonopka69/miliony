...
export interface Place {
    ...
    types: string[];
    searchType?: string; // Category used to search (for fallback icon display)
}
...
export const SPORT_CATEGORY_RULES: Record<string, FilterRule> = {
    ...
    soccer: {
        primaryType: 'point_of_interest',
        requiredKeywords: ['soccer', 'football', 'piłka nożna', 'boisko', 'stadion', 'orlik'],
        excludedTypes: ['store', 'parking', 'bar', 'restaurant'],
        excludedNamePatterns: [/store/i, /shop/i, /equipment/i],
        description: 'Soccer and football fields'
    },

    volleyball: {
        primaryType: 'point_of_interest',
        requiredKeywords: ['volleyball', 'siatkówka', 'beach volleyball', 'boisko'],
        excludedTypes: ['store', 'parking'],
        excludedNamePatterns: [/store/i, /shop/i, /equipment/i],
    ...
    yoga: {
        primaryType: 'gym',
        requiredKeywords: ['yoga', 'joga', 'pilates', 'studio'],
        excludedTypes: ['store', 'parking', 'restaurant'],
        excludedNamePatterns: [/store/i, /shop/i],
        description: 'Yoga and pilates studios'
    },

    climbing: {
        primaryType: 'gym',
        requiredKeywords: ['climbing', 'bouldering', 'wspinaczka', 'ścianka'],
        excludedTypes: ['store', 'parking'],
        excludedNamePatterns: [/store/i, /shop/i, /equipment/i],
        description: 'Climbing gyms and walls'
    },
...
class PlacesApiService {
    ...
    async searchNearby(
        ...
        try {
            ...
            // Maps user favorite sports to search categories
            const keywordToCategoryMap: Record<string, string> = {
                // Polish keywords
                'siłownia': 'gym',
                'kalistenika': 'gym',
                'piłka nożna': 'soccer',
                'koszykówka': 'basketball_court',
                'siatkówka': 'volleyball',
                'tenis': 'tennis_court',
                'boks': 'martial_arts_gym',
                'sztuki walki': 'martial_arts_gym',
                'pływanie': 'swimming_pool',
                'joga': 'yoga',
                'wspinaczka': 'climbing',
                'lodowisko': 'ice_rink',
                // English keywords
            ...
            // Detect category from keywords if types is empty
            let category = 'all';
            let detectedCategories: string[] = [];

            // If multiple types are provided, search all of them
            if (filter.types.length > 1) {
                console.log(`🔄 Multiple filter types detected: [${filter.types.join(', ')}]`);
                console.log(`🔄 Searching ALL types...`);

                for (const type of filter.types) {
                    const typeRules = SPORT_CATEGORY_RULES[type];
                    if (typeRules) {
                        console.log(`\n📋 Searching type: ${type}`);
                        const typeResults = await this.searchByCategory(location, filter, type, typeRules);
                        console.log(`   Found ${typeResults.length} results for ${type}`);
                        allResults.push(...typeResults);
                    } else {
                        console.log(`⚠️ No rules for type: ${type}, trying fallback...`);
                        // Try keyword-based search for unrecognized types
                        const fallbackResults = await this.searchByKeywordWithType(location, type, filter);
                        console.log(`   Found ${fallbackResults.length} fallback results for ${type}`);
                ...
                console.log(`\n📊 Total unique results from all types: ${uniqueResults.length}`);
                performanceOptimizer.setCache(cacheKey, uniqueResults, 5 * 60 * 1000);
                return uniqueResults;
            } else if (filter.types.length > 0) {
                category = filter.types[0];
            } else if (filter.keywords.length > 0) {
                // Try to detect categories from keywords
                for (const keyword of filter.keywords) {
                    const lowerKeyword = keyword.toLowerCase();
                    for (const [key, cat] of Object.entries(keywordToCategoryMap)) {
                ...
                if (detectedCategories.length > 0) {
                    category = detectedCategories[0]; // Use first detected category
                    console.log(`🎯 Detected categories from keywords: [${detectedCategories.join(', ')}]`);
                }
            }

            console.log(`🔑 Category detected: "${category}"`);

            // If we have multiple categories from keywords, search all of them
            if (detectedCategories.length > 1) {
                console.log(`🔄 Searching multiple categories: [${detectedCategories.join(', ')}]`);

                for (const cat of detectedCategories) {
                    const catRules = SPORT_CATEGORY_RULES[cat];
                    if (catRules) {
                        console.log(`\n📋 Searching category: ${cat}`);
                        const catResults = await this.searchByCategory(location, filter, cat, catRules);
                ...
                console.log(`\n📊 Total unique results from all categories: ${uniqueResults.length}`);
                performanceOptimizer.setCache(cacheKey, uniqueResults, 5 * 60 * 1000);
                return uniqueResults;
            }
            ...
            const mappedResults: Place[] = validatedResults.map((place: any) => ({
                ...
                types: place.types || [],
                searchType: category, // Category used to search (for fallback icon display)
    ...
    // Helper method to search a single category with rules
    private async searchByCategory(
        ...
        console.log(`📋 Searching category: ${category} (${rules.description})`);
        ...
        try {
            ...
            if (data.status !== 'OK') {
                console.log(`⚠️ No results for ${category}: ${data.status}`);
                return [];
            }

            const rawResults = data.results || [];
            console.log(`📦 Got ${rawResults.length} raw results for ${category}`);

            // Apply validation - but be less strict for keyword-based searches
            const validatedResults: any[] = [];
            ...
            console.log(`✅ Validated ${validatedResults.length}/${rawResults.length} for ${category}`);

            // Map to our format
            return validatedResults.map((place: any) => ({
                placeId: place.place_id,
                name: place.name,
                address: place.vicinity || place.formatted_address || 'Address not available',
                coordinates: {
