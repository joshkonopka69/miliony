-- COMPLETE FIX FOR YOUR APP - RUN THIS SQL IN SUPABASE DASHBOARD
-- This will disable RLS temporarily, create sample data, and fix all issues
...
-- Step 3: Create sample events with proper structure
...
-- Step 10: Re-enable RLS with proper policies
...
-- Step 11: Create proper RLS policies for events
...
-- Step 12: Create proper RLS policies for event_participants
...
-- Step 13: Create proper RLS policies for event_messages
...
-- Step 14: Create proper RLS policies for users
CREATE POLICY "Users can view all profiles" ON users FOR SELECT USING (true);
CREATE POLICY "Users can update own profile" ON users FOR UPDATE USING (auth.uid() = id);

-- Step 15: Create proper RLS policies for notifications
CREATE POLICY "Users can view own notifications" ON notifications FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can update own notifications" ON notifications FOR UPDATE USING (auth.uid() = user_id);

-- Step 16: Create proper RLS policies for privacy_settings
...
-- Step 17: Create proper RLS policies for groups
...
-- Step 18: Create proper RLS policies for group_members
...
('Test Event', 'Basketball', 'Test event to verify functionality', 10, 51.1079, 17.0385, 'Test Location', 'c46dec97-bfd3-4d30-9cc8-178b1a2b66a7', NOW() + INTERVAL '1 day', 'active');

SELECT 'Test event created successfully!' as result;
