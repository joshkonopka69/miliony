-- QUICK FIX - Run this first to resolve the NOT NULL constraint error
...
-- Step 6: Test that event creation now works
INSERT INTO events (title, sport_type, max_participants, latitude, longitude, created_by, scheduled_datetime, status) VALUES
('Test Event - Fixed', 'Basketball', 10, 51.1079, 17.0385, 'c46dec97-bfd3-4d30-9cc8-178b1a2b66a7', NOW() + INTERVAL '1 day', 'active');

-- Step 7: Verify
SELECT 'Events created successfully!' as result, COUNT(*) as count FROM events;
SELECT 'Event participants created!' as result, COUNT(*) as count FROM event_participants;
SELECT 'Event messages created!' as result, COUNT(*) as count FROM event_messages;

-- Step 8: Re-enable RLS with basic policies
